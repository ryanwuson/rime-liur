...
預設每頁 **5** 個（選字鍵 `0`～`4`）。嘸蝦米、獨立注音、獨立拼音、Easy English **只改一處** 就四處一起變。

開啟使用者資料夾的 `default.custom.yaml`，在 `menu:` 裡找到：

```yaml
        page_size: 5	#選單每頁個數（嘸蝦米／注音／拼音／英文只改這裡）
```

把 `5` 改成 `7` 或 `9`，存檔後重新部署。選字鍵已是 `0`～`9`（從 `0` 開始），改成 7 後用 `0`～`6`，改成 9 後用 `0`～`8`。

?> 不要改 `liu_bpmf.schema.yaml`／`liu_pinyin.schema.yaml`。方案裡若再寫 `menu/page_size`，會蓋掉這一處，改 7 會看起來沒效。
